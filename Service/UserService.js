...
async function signup(req, res) {
    ...
    if (!(UserName && Email && Password && Role)) {
        ...
        if (oldUser) {
            ...
            let token;
            token = jwt.sign({ userId: createdUser._id },
            ...
            return {
                ...
                StartupName: createdUser.StartupName,
                ImageProfile: createdUser.ImageProfile,
                Cv: createdUser.Cv,
                Typecreator: createdUser.Typecreator,
...
async function login(req, res) {
    ...
    res.json({
        ...
        Cv: existingUser.Cv,
        Typecreator: existingUser.Typecreator,
                ...
                if (err) {
                    ...
                    if (user) {
                        ...
                        token = jwt.sign({ userId: user._id, Email: user.Email },
                            'supersecret_dont_share', { expiresIn: '3h' }
                        );
                        ...
                        newUser.save((err, data) => {
                            ...
                            let token;
                            token = jwt.sign({ userId: data._id, Email: data.Email },
                                'supersecret_dont_share', { expiresIn: '3h' }
                            );
...
async function  displayAllInvestors(){
   return  await User.find({Role:"Investor"}).then(data=>{ console.log(data);})
...
module.exports = { LoginWithFacebook, LoginWithGoogle, addUser, displayUserById, updateUser, deleteUserById, displayAllUser, displayAllAdmin, displayAllUsersExceptAdmin, signup, login, updateSimpleUser, updateInvestor, change_password,displayAllInvestors }